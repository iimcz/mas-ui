<script lang="ts">
    ...
    let {
        ...
        onLink,
        linkedObjects
    }: { paratextId: string; onLink: () => void; linkedObjects: DigitalObject[] } = $props();

    let digitalObjects = $state<DigitalObject[] | null>(null);
    ...
    async function fetchDigitalObjects() {
        loading = true;
        try {
            const res = await fetch(`${API_URL}/api/v1/digitalobjects`);
            if (!res.ok) {
                toaster.error({ title: "Načtení digitálních objektů se nezdařilo" });
                return;
            }

            return (await res.json()) as DigitalObject[];
        } catch {
            toaster.error({ title: "Načtení digitálních objektů se nezdařilo" });
        } finally {
            loading = false;
    ...
    async function linkDigitalObject(id: string) {
        linkingId = id;
        try {
            const res = await fetch(`${API_URL}/api/v1/digitalobjects/${id}/link`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ paratextId })
            });
            if (!res.ok) {
                toaster.error({ title: "Propojení se nezdařilo" });
                return;
            }
            toaster.success({ title: "Digitální objekt byl propojen" });
            onLink();
        } catch {
            toaster.error({ title: "Propojení se nezdařilo" });
        } finally {
        ...
        <Dialog.Positioner class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
                <header class="flex items-center justify-between">
                    <Dialog.Title class="text-lg font-bold">Propojit digitální objekt</Dialog.Title>
