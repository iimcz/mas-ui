        ...
        {#if matchingMediaPlatforms.length != 0}
            ...
            <div class="grid grid-cols-3 gap-2 text-center">
                {#each matchingMediaPlatforms as platform}
                    <EmulatorCard
                        on:select={() => goto(platform.name)}
                        title={platform.name}
                        description={platform.physicalMediaTypes
                            .map((m) => $_(`media_type.${m}`))
                            .join(", ")}
                    />
                {/each}
            </div>
        {/if}
        <h2 class="mt-4 text-xl">Seznam platforem</h2>
